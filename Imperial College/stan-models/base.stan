data {
  ...
  int casos[N2,M]; // reported casos
  int mortes[N2, M]; // reported mortes -- the rows with i > N contain -1 and should be ignored
  matrix[N2, M] f; // h * s
...
transformed parameters {
    ...
    matrix[N2, M] E_mortes  = rep_matrix(0,N2,M);
    matrix[N2, M] Rt = rep_matrix(0,N2,M);
    for (m in 1:M){
      prediction[1:N0,m] = rep_vector(y[m],N0); // learn the number of casos in the first N0 days
      ...
      for (i in 2:N2){
        E_mortes[i,m]= 0;
        for(j in 1:(i-1)){
          E_mortes[i,m] += prediction[j,m]*f[i-j,m];
        }
      }
    }
   /* for(m in 1:M) {
     for(i in 1:N[m]) {
      LowerBound[i,m] = prediction[i,m] * 10 - casos[i,m];
     }
    }*/
...
model {
  ...
  for(m in 1:M){
    for(i in EpidemicStart[m]:N[m]){
       mortes[i,m] ~ neg_binomial_2(E_mortes[i,m],phi); 
    }
   }
}
...
generated quantities {
    ...
    matrix[N2, M] E_mortes0  = rep_matrix(0,N2,M);
    for (m in 1:M){
      prediction0[1:N0,m] = rep_vector(y[m],N0); // learn the number of casos in the first N0 days
      for (i in (N0+1):N2) {
        convolution0=0;
        for(j in 1:(i-1)) {
      ...
      for (i in 2:N2){
        E_mortes0[i,m]= 0;
        for(j in 1:(i-1)){
          E_mortes0[i,m] += prediction0[j,m]*f[i-j,m];
        }
      }
      for(i in 1:N[m]){
        lp0[i,m] = neg_binomial_2_lpmf(mortes[i,m] | E_mortes[i,m],phi); 
        lp1[i,m] = neg_binomial_2_lpmf(mortes[i,m] | E_mortes0[i,m],phi); 
      }
    }
}
